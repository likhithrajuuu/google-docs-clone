    ...
    const handleMouseMove = (e : React.MouseEvent) =>{
        if((isDraggingLeft || isDraggingRight) && rulerRef.current){
            ...
            if(container){
                ...
                const rawPostition = Math.max(0, Math.min(816, relativeX));

                if(isDraggingLeft){
                    const maxLeftPosition = 816 - rightMargin - 100;
                    const newLeftPosition = Math.min(rawPostition, maxLeftPosition);
                    setLeftMargin(newLeftPosition); //TODO: Make Collaborative
                }
                else if(isDraggingRight){
                    const maxRightPosition = 816 -(leftMargin + 100);
                    const newRightPosition = Math.max(816 - rawPostition, 0);
...
const Marker = ({
    ...
    >
        <FaCaretDown className="absolute left-1/2 top-0 h-full fill-blue-500 transform -translate-x-1/2 group-hover:fill-blue-600"/>
